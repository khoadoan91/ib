...
const fad_data_type_1 = __importDefault(require("../../api/data/enum/fad-data-type"));
  ...
  function tagValuesToTokens(tagValues: TagValue[]): unknown[] {
    ...
    placeOrder(id, contract, order) {
        ...
        if (this.serverVersion >= 19) {
            ...
            const lower = this.serverVersion === 26 && order.orderType === "VOL"
                ? undefined
                : order.stockRangeLower;
            const upper = this.serverVersion === 26 && order.orderType === "VOL"
                ? undefined
        ...
        if (this.serverVersion >= 26) {
            ...
            if (this.serverVersion === 26) {
                // Volatility orders had specific watermark price attribs in server version 26
                const lower = order.orderType === "VOL" ? order.stockRangeLower : undefined;
                const upper = order.orderType === "VOL" ? order.stockRangeUpper : undefined;
        ...
        if (this.serverVersion >= min_server_version_1.default.PEGBEST_PEGMID_OFFSETS) {
            ...
            if ((0, orderType_1.isPegBestOrder)(order.orderType)) {
                ...
                if (order.competeAgainstBestOffset ==
                    order_1.COMPETE_AGAINST_BEST_OFFSET_UP_TO_MID)
                    sendMidOffsets = true;
            }
